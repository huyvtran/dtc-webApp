import { ChooseInvoicePage } from './../../orders/choose-invoice/choose-invoice';
import { ChooseAddressPage } from './../../orders/choose-address/choose-address';
import { InvoiceStatePage } from './../../customer/invoice-state/invoice-state';
import { NewaddPage } from './../../customer/newadd/newadd';
import { OrderAddressPage } from './../../orders/order-address/order-address';
import { OnlineCustomerPage } from './../../orders/online-customer/online-customer';
import { PaymentVoucherPage } from '../../orders/payment-voucher/payment-voucher';
import { QrCodePage } from '../../orders/qr-code/qr-code'
import { ReceiptPage } from '../../orders/receipt/receipt'
...
/**
 * Generated class for the ActivityDeliverPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-activity-deliver',
  templateUrl: 'activity-deliver.html',
})
export class ActivityDeliverPage {
  ...
  youhui() {
    ...
    for (let i = 0; i < this.shopping.length; i++) {
      ...
      this.userInfo.totalPay = this.total * 100;
      this.userInfo.productMoney = this.total * 100;
      this.userInfo.orderMoney = this.total * 100;
  ...
  updateDiscount() {
    ...
    let data = {
      ...
      discountAmount: this.userInfo.discountAmount,
    ...
    if (this.shopping.length > 0) {
      for (let i = 0; i < this.shopping.length; i++) {
        if (this.shopping[i].productId == id) {
          if (this.shopping[i].counts >= this.shopping[i].stock) {
  ...
  //礼品
  togift() {
    this.navCtrl.push(ActiviGiftPage, { resolve: this.callback, id:this.userInfo.activityId})
  }
  // 支付方式
  payment(type) {
    if (type == '微信支付') {
  ...
  button() {
    ...
    console.log(this.giftList, this.userInfo.productList)
    if (!this.buyerName) {
  ...
  tijiao() {
    ...
    if (this.userInfo.discountAmount > 0) {
      if (!this.userInfo.dtcName) {
        this.httpService.showToast('请选择审批对象');
        return;
      }
    }

    this.userInfo.discountAmount = this.userInfo.discountAmount * 100;
    this.httpService.posts(AppConfig.API.online, this.userInfo).subscribe(res => {
      console.log(res)
      let code = res.code
      if (code === 100000) {
